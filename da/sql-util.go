...
const SQL_BASIC_JOIN = "track s JOIN listen l ON s.id = l.trackID"

const SQL_TOP_TRACKS = "SELECT s.name, s.id, s.artist, s.artistID, a.image, COUNT(*) FROM " + SQL_BASIC_JOIN + " JOIN album a ON s.album = a.id WHERE username = ? AND timestamp >= ? AND timestamp <= ? GROUP BY s.id ORDER BY COUNT(*) DESC LIMIT 1000;"
const SQL_TOP_TRACKS_TIME = "SELECT s.name, s.id, s.artist, s.artistID, a.image, ROUND(COUNT(*) * s.duration / 1000) AS time FROM " + SQL_BASIC_JOIN + " JOIN album a ON s.album = a.id WHERE username = ? AND timestamp >= ? AND timestamp <= ? GROUP BY s.id ORDER BY time DESC LIMIT 1000;"
...
const SQL_UNIQUE_TRACKS = "SELECT COUNT(DISTINCT s.id) FROM " + SQL_BASIC_JOIN + " WHERE username = ? AND timestamp >= ? AND timestamp <= ?;"
